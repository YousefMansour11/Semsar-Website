...
function SectionHeader({ label, accent, icon }: { label: string; accent?: string; icon?: React.ReactNode }) {
  return (
    <div>
...
function FilterChips({ filters, onClear }: { filters: SearchFilters; onClear: (key: string) => void }) {
  ...
  if (filters.locationIds?.length) chips.push({ key: 'locationIds', label: t('filters.location') });
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ── Featured Projects ─────────────────────── */}
      <section id="projects" className="py-12 sm:py-24 bg-muted/30">
      ...
      {/* ── Land Request CTA ──────────────────── */}
      <section id="land-request" className="py-12 sm:py-24 bg-background">
          ...
          <div className="reveal-on-scroll relative bg-gradient-to-r from-gold to-gold-dark rounded-2xl sm:rounded-3xl p-8 sm:p-10 md:p-16 text-center overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <button
                ...
                className="px-8 sm:px-10 py-3 sm:py-4 bg-white text-navy rounded-full font-bold text-base sm:text-lg shadow-2xl shadow-black/20 hover:bg-gray-100 hover:-translate-y-1 transition-all duration-300"
              >
                {t('land.cta')}
              </button>
            </div>
      ...
      {/* ── Properties for Sale ───────────────────── */}
      <section id="for-sale" className="py-12 sm:py-24 bg-muted/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <FilterChips filters={saleFilters} onClear={(key) => setSaleFilters(prev => {
            const next = { ...prev };
      ...
      {/* ── Properties for Rent ───────────────────── */}
      <section id="for-rent" className="py-12 sm:py-24 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 mb-8 sm:mb-12">
            <div className="flex items-end gap-4">
          ...
          <FilterChips filters={rentFilters} onClear={(key) => setRentFilters(prev => {
            const next = { ...prev };
            if (key === 'price') { delete next.minPrice; delete next.maxPrice; }
            else if (key === 'size') { delete next.minSize; delete next.maxSize; }
            else delete (next as SearchFilters)[key as keyof SearchFilters];
