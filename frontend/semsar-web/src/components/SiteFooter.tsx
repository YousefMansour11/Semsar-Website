...
export const SiteFooter = memo(function SiteFooter() {
  ...
  const showProjects = location.pathname === '/' || location.pathname.startsWith('/projects');
  ...
  ].filter(s => s.href), [social.facebook, social.instagram, wa]);
              ...
              <div className="flex items-center gap-2.5 mt-4 sm:mt-5">
                {socialLinks.map(({ href, icon, label }) => (
                  <a
                    ...
                    aria-label={`${label}${label === 'WhatsApp' ? ' (Opens in new tab)' : ''}`}
                    className="w-11 h-11 rounded-full border border-white/15 flex items-center justify-center text-white/40 hover:text-white hover:border-gold/60 hover:bg-gold/10 transition-colors duration-300"
                  >
                    {icon}
                  </a>
                ))}
              ...
              {quickLinks.map((item) => (
                <li key={item.label}>
                  {'to' in item ? (
                    ...
                    <button type="button" onClick={() => navigate(localizedPath('/', language), { state: { scrollTo: 'scrollTo' in item ? item.scrollTo : null } })} className="text-white/55 hover:text-white text-xs sm:text-sm transition-colors duration-200 text-start py-1">{item.label}</button>
                  )}
                </li>
              ))}
            </ul>
