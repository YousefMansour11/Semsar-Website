...
const CascadingLocationPicker = React.memo(function CascadingLocationPicker({ value, onChange, onLocationIdsChange }: Props) {
  ...
  const handleSelect = useCallback((node: LocationTreeNode) => {
    if (browsingLevel === 'gov') {
      ...
      onLocationIdsChange(newAreaIds.length ? newAreaIds : (value.cityId != null ? [value.cityId] : []));
      setSearch('');
      inputRef.current?.blur();
      setIsFocused(false);
    }
  }, [browsingLevel, value, onChange, onLocationIdsChange]);

  const clearLevel = useCallback((level: 'gov' | 'city' | 'area') => {
    if (level === 'gov') {
      onChange({ areaIds: [] });
      onLocationIdsChange([]);
    ...
    setSearch('');
    setIsFocused(true);
  }, [value, onChange, onLocationIdsChange]);

  const removeArea = useCallback((areaId: number) => {
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 mb-3">
        {value.governorateId && govNode && (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-secondary/10 text-secondary text-xs font-semibold">
            ...
            <button type="button" onClick={() => clearLevel('gov')} aria-label={language === 'ar' ? 'إزالة' : 'Clear'} className="hover:bg-secondary/20 rounded p-0.5">
              <X className="w-3 h-3" />
            </button>
          </span>
        )}
        {value.cityId && cityNode && (
          <>
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary/10 text-primary text-xs font-semibold">
              ...
              <button type="button" onClick={() => clearLevel('city')} aria-label={language === 'ar' ? 'إزالة' : 'Clear'} className="hover:bg-primary/20 rounded p-0.5">
                <X className="w-3 h-3" />
              </button>
            </span>
          </>
        ...
        {selectedAreaNodes.map(areaNode => (
          <span key={areaNode.id} className="flex items-center gap-0">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-xs font-semibold">
              ...
              <button type="button" onClick={() => removeArea(areaNode.id)} aria-label={language === 'ar' ? 'إزالة' : 'Clear'} className="hover:bg-amber-200 dark:hover:bg-amber-800/30 rounded p-0.5">
                <X className="w-3 h-3" />
              </button>
            </span>
          </span>
      ...
      <div className="relative mb-2">
        ...
        <input id="clp-search" name="locationSearch"
          aria-label={browsingLevel === 'gov' ? t('filters.searchGovernorate') : browsingLevel === 'city' ? t('filters.searchCity') : t('filters.searchArea')}
          ref={inputRef}
          type="text"
          ...
          className={`w-full bg-muted/50 border border-border rounded-xl ${language === 'ar' ? 'pr-9 pl-3' : 'pl-9 pr-3'} py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary/30 focus:border-secondary transition-shadow`}
          autoComplete="off"
        />
      </div>
      ...
      {showDropdown && (
        <div className="max-h-48 overflow-y-auto overflow-x-hidden space-y-1 rounded-xl border border-border bg-card" role="listbox">
          ...
          {filteredOptions.map(node => {
            ...
            return (
              <button
                ...
                aria-expanded={node.children.length > 0 ? false : undefined}
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="truncate">{language === 'ar' ? (node.nameAr || node.nameEn) : node.nameEn}</span>
                </span>
...
export { CascadingLocationPicker };
