...
export const AdvancedFilterPanel = memo(function AdvancedFilterPanel({ filters, onApply, priceSuffix = 'EGP' }: Props) {
  ...
  const openPanel = useCallback(() => {
    ...
    setOpen(true);
  }, [filters, locationTree]);

  const applyAndClose = useCallback(() => {
    const appliedLocationIds = locationSelection.areaIds.length
      ? locationSelection.areaIds
      : locationSelection.cityId
        ? [locationSelection.cityId]
        : locationSelection.governorateId
          ? [locationSelection.governorateId]
          : undefined;
    const merged = { ...draft, locationIds: appliedLocationIds };
    onApply(merged);
    setOpen(false);
  }, [locationSelection, draft, onApply]);
  ...
  const set = useCallback((key: keyof SearchFilters, value: SearchFilters[keyof SearchFilters]) =>
  ...
  return (
    <>
      <button
        ...
        className={`min-h-[44px] flex items-center gap-2 px-5 py-3 rounded-xl font-semibold text-sm border transition-colors duration-200 ${
          active
      ...
      {open && (
        <>
          ...
          <div ref={focusTrapRef} className={`fixed inset-y-0 ${language === 'ar' ? 'left-0 animate-in slide-in-from-left' : 'right-0 animate-in slide-in-from-right'} duration-300 ease-out z-50 w-full max-w-sm bg-card shadow-2xl shadow-black/10 flex flex-col`} role="dialog" aria-modal="true" aria-label={t('filters.title')}>
            <div className="flex items-center justify-between p-6 border-b border-border">
              <div className="flex items-center gap-2">
                <SlidersHorizontal className="w-5 h-5 text-secondary" />
                <h2 className="text-lg font-bold">{t('filters.title')}</h2>
              </div>
              <button type="button" onClick={() => setOpen(false)} aria-label={t('filters.close')} className="p-2 rounded-lg hover:bg-muted transition-colors" autoFocus>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div>
                ...
                <select id="afp-sortBy" name="sortBy"
                  ...
                  className="w-full bg-muted/50 border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary/30 focus:border-secondary transition-shadow"
                >
            ...
            <div className="p-4 sm:p-6 border-t border-border flex gap-3">
              <button type="button" onClick={resetAndClose} className="flex-1 min-h-[44px] py-3 rounded-xl border border-border font-semibold text-sm hover:bg-muted/60 transition-colors duration-200 active:scale-[0.97]">
                {t('filters.reset')}
              </button>
              <button type="button" onClick={applyAndClose} className="flex-1 min-h-[44px] py-3 rounded-xl bg-secondary text-white font-semibold text-sm hover:bg-secondary/90 shadow-lg shadow-secondary/20 transition-colors duration-200 active:scale-[0.97]">
                {t('filters.apply')}
              </button>
            </div>
          </div>
