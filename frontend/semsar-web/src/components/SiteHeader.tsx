...
export const Header = memo(function Header() {
  ...
  const navLinks = useMemo(() => [
    { label: t('nav.home'), href: '/', isRoute: true as const },
    { label: t('nav.projects'), href: '#projects', isRoute: false as const },
    { label: t('nav.forSale'), href: '#for-sale', isRoute: false as const },
    { label: t('nav.forRent'), href: '#for-rent', isRoute: false as const },
    { label: t('nav.lands'), href: '#land-request', isRoute: false as const },
  ], [t]);
  ...
  return (
    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-[background-color,box-shadow,padding] duration-400 ${
        showSolid
          ? 'bg-white/95 backdrop-blur-xl shadow-sm border-b border-gray-100/80 py-2 sm:py-3 after:content-[""] after:absolute after:bottom-0 after:left-1/4 after:right-1/4 after:h-[1px] after:bg-gradient-to-r after:from-transparent after:via-gold/30 after:to-transparent'
          : 'bg-gradient-to-b from-black/20 to-transparent py-3 sm:py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-[max(env(safe-area-inset-top,0px),0px)]">
        <div className="flex items-center justify-between gap-4">
          <Link to={localizedPath('/', language)} className="flex items-center gap-2 group shrink-0">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center p-1 transition-[width,height,padding] duration-300 ${
              showSolid
                ? 'bg-[#0A1628]'
          ...
          <nav className="hidden md:flex items-center gap-0.5 flex-1 justify-center" aria-label={t('nav.home')}>
            {navLinks.map(({ label, href, isRoute }) => {
              const isActive = isRoute && stripLanguagePrefix(location.pathname) === href;
              return isRoute ? (
                <Link
                  key={href}
                  to={localizedPath(href, language)}
                  aria-current={isActive ? 'page' : undefined}
                  className={`relative px-3 lg:px-4 py-2 text-sm font-medium transition-colors duration-200 group rounded-lg ${
                ...
                <a
                  ...
                  className={`relative px-3 lg:px-4 py-2 text-sm font-medium transition-colors duration-200 group rounded-lg cursor-pointer ${
                    showSolid ? 'text-gray-500 hover:text-navy' : 'text-white/75 hover:text-white'
                  }`}
                >
                  {label}
            ...
            <div className={`flex items-center gap-0.5 p-1 rounded-xl transition-colors ${
              ...
              {languages.map(lang => (
                <button
                  ...
                  className={`text-xs font-bold px-3 min-h-[44px] rounded-lg transition-colors duration-200 ${
                    language === lang.code
          ...
          <button
            className={`md:hidden min-h-[44px] min-w-[44px] flex items-center justify-center rounded-xl border transition-colors duration-200 ${
              showSolid
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out ${
        isMobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 invisible'
      }`} aria-hidden={!isMobileMenuOpen} inert={!isMobileMenuOpen ? "true" : undefined}>
