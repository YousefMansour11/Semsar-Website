...
const stableFilters = new WeakMap<SearchFilters, string>();
function getFiltersKey(filters: SearchFilters): string {
  let key = stableFilters.get(filters);
  if (!key) {
    key = JSON.stringify(filters);
    stableFilters.set(filters, key);
  }
  return key;
}
...
export function usePropertySearch(filters: SearchFilters) {
  return useQuery({
    queryKey: ['property-search', getFiltersKey(filters)],
